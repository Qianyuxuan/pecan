...
phenology_MODIS_extract<- function(settings,run_parallel = TRUE,ncores = NULL){ 
  ...
  if (!is.null(outdir)) {
    ...
    cycle <- PEcAn.data.remote::call_MODIS(outdir = NULL, 
                                             var = "QA", 
                                             site_info = site_info, 
                                             product_dates = c(start_YEARDOY, end_YEARDOY),
                                             run_parallel = as.logical(run_parallel), 
    ...
    leafphdata$leafonday<-as.Date(leafphdata$leafonday)#convert the dates to Day-of-Year format
    leafphdata$leafoffday<-as.Date(leafphdata$leafoffday)
    leafphdata$leafonday<-lubridate::yday(gsub(16,17,leafphdata$leafonday))
    leafphdata$leafoffday<-lubridate::yday(gsub(16,17,leafphdata$leafoffday))
